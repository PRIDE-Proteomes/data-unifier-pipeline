...
printUsage() {
    ...
    echo ""
    echo "Usage: ./runDataUnifier.sh [-e|--email] "

    echo "     Example: ./runDataUnifier.sh -e ntoro@ebi.ac.uk -d test -i prod"
...
#bsub  -q ${QUEUE} -e ${STD_ERR} -o ${STD_OUT} -M ${MEMORY_LIMIT} -J ${JOB_NAME} -N -u ${JOB_EMAIL} java -Xmx${MEMORY_LIMIT}m -DDB_ENVIRONMENT=${DB_ENV} -INDEX_ENVIRONMENT=${INDEX_ENV} ${COMMAND} ${JOB_PARAMETERS}
