...
JOB_NAME="proteomes-data-unifier-indexer"
...
# LSF command to run
COMMAND="-jar ${project.build.finalName}.jar launch-data-unifier-indexer-job.xml proteomesDataUnifierIndexerJob"
...
##### FUNCTIONS
printUsage() {
    echo "Description: Data unifier indexer pipeline."
    echo ""
    echo "Usage: ./runDataUnifierIndexer.sh -i|--index -d|--database [-e|--email] [-j|--job-params]"

    echo "     Example: ./runDataUnifierIndexer.sh -e ntoro@ebi.ac.uk -d test -i prod"
